...
CameraParameters::CameraParameters(CameraParameters *p)
    : mParams(p)
{
}
...
int CameraParameters::get_from_attr(const char *path, char *buf, size_t len)
{
    FILE *fp;
    int ret = 0;
...
bool CameraParameters::check_flashlight_restriction()
{
    /* Just lie and always say we are able to use the flashlight */
    return false;
}

int CameraParameters::lookupAttr(/* CameraParameters::CameraMap const* */
...
void CameraParameters::setZsl(const char *mode)
{
    mParams->set("zsl", mode);
}

const char * CameraParameters::getZsl() const
{
    return mParams->get("zsl");
}

void CameraParameters::setRawSize(int x, int y)
{
    char buf[32];
    snprintf(buf, 32, "%dx%d", x, y);
...
void CameraParameters::getRawSize(int *x, int *y)
{
    const char *p;
    *x = *y = -1;
